...
namespace AuthServer.Web.HostedServices;
...
public sealed class OpenIddictWorker(IServiceProvider serviceProvider) : IHostedService
{
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        const string botClientId = "telegram-bot";
        if (await manager.FindByClientIdAsync(botClientId, cancellationToken) is null)
        {
            await manager.CreateAsync(new OpenIddictApplicationDescriptor
            {
                ClientId = botClientId,
                ClientSecret = "super-secret",
                DisplayName = "Telegram Bot Client",
                Permissions =
                {
                    // Разрешаем токен-эндпоинт
                    OpenIddictConstants.Permissions.Endpoints.Token,

                    // Разрешаем client_credentials flow
        ...
        {
            var url = serviceProvider.GetRequiredService<IConfiguration>().GetValue<string>("Auth:Url");
            ...
            await manager.CreateAsync(new OpenIddictApplicationDescriptor
            {
                ...
                RedirectUris = {
                    ...
                    new Uri("https://localhost:30001/callback/login/local")             // Calabonga.Microservices.RazorPagesUI
                },
